...
def generate_with_step_vqa_gd(
    ...
    for i, t in enumerate(scheduler.timesteps):
        ...
        на каждой итерации
            ...
            - если делаем GD, то на каждой его итерации:
                - сохранить ✅ x_0i->img before gd step
                            ✅ vqa-score_i before gd step
                            ✅ x_ti->img before gd step
                            ✅ x_t-difference_i->img
                
            - после GD сохраить x_t->img before denoise step
                                ✅ x_0->img before denoise step
                                ✅ vqa-score before denoise step
            
            - ✅ сделать шаг разшумления
        
        - ✅ в самом конце final vqa-score
        ...
        if do_gd_this_step:
            for gd_iter in range(gd_steps):
                ...
                with torch.no_grad():
                    x0_img_path = os.path.join(x0_dir, f"{filename_base}_gd_{gd_iter + 1:02d}_before_gd_step.png")
        ...
        with torch.no_grad():
            ...
            xt_path = os.path.join(
                xt_dir,
                f"{filename_base}_done_gd_step.png"
            )
            save_image_tensor(xt_img_before, xt_path)
                    
        # vanilla denoising step
        with torch.no_grad():
            eps_pred = predict_eps_with_cfg(
                x_t=x_t_gd,
                t=t,
        ...
        record = {
            ...
            "gd_stats": gd_records if do_gd_this_step else None,
        }
        step_records.append(record)

        if do_gd_this_step:
            print(
                f"step={i:03d} | t={t:04d} | gd_applied={do_gd_this_step} | "
                f"VQA(x0 before GD)={first_vqa:.2f} | "
                f"VQA(x0 after GD)={vqa_after_gd:.2f} | "
            )
        else:
            print(
                f"step={i:03d} | t={t:04d} | gd_applied={do_gd_this_step} | "
...
if __name__ == "__main__":
    ...
    # prompt = "A bear does a handstand in the park"
    
    prompt = "a bird with three legs"
